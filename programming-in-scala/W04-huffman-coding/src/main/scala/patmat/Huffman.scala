...
object Huffman {
  ...
  def decode(tree: CodeTree, bits: List[Bit]): List[Char] = {
    def traverse(remain: CodeTree, bits: List[Bit]): List[Char] = remain match {
  ...
  def codeBits(table: CodeTable)(char: Char): List[Bit] = {
    table
      .find({ case (c, _) => c == char })
      .map({ case (_, bytes) => bytes })
      .getOrElse(Nil)
  }
  ...
  def mergeCodeTables(left: CodeTable, right: CodeTable): CodeTable = {
    def step(x: Int, code: Code): Code = code match {
      case (char, bytes) => (char, x :: bytes)
    }

    left.map(step(0, _)) ::: right.map(step(1, _))
  }
